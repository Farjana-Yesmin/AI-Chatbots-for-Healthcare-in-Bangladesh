...
# Define skf, X_clim, and y_clim moved from later cells
skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
...
X_clim_copy['MAX'] = X_clim_copy['MAX'].fillna(mean_max)
X_clim_copy['RAINFALL'] = X_clim_copy['RAINFALL'].fillna(mean_rainfall)
...
cases = [101354, 25216, 36201, 48962, 321179]
plt.plot(years, cases, marker='o', color='#1f77b4', linewidth=2)
plt.fill_between(years, cases, alpha=0.2, color='#1f77b4')
plt.title("Dengue Cases in Bangladesh (2019-2023)")
plt.xlabel("Year")
plt.ylabel("Number of Cases")
plt.grid(True)
plt.savefig("Dengue_Cases_Trend.png", dpi=300, bbox_inches='tight')
plt.close()

# ============================================================
# CELL: SHAP Explainability Analysis
# Add this after your existing Feature Importance cell
# ============================================================
!pip install shap -q
...
# SHAP TreeExplainer on the non-leaky model
explainer = shap.TreeExplainer(best_clf_sub)
shap_values = explainer.shap_values(X_test_sub)

# Use encoded names from X_sub for accurate labeling
feature_names = ['Gender_encoded', 'Age', 'AreaType_encoded', 'HouseType_encoded', 'District_encoded']

# Robustly get SHAP values for the positive class (class 1)
# This handles cases where shap_values is a list of arrays (standard for TreeExplainer classification)
# or a direct 3D array (samples, features, classes) if it somehow ends up that way.
if isinstance(shap_values, list):
    shap_vals_for_plotting = shap_values[1] # For binary classification, index 1 is usually the positive class
elif shap_values.ndim == 3:
    shap_vals_for_plotting = shap_values[:, :, 1]
else:
    shap_vals_for_plotting = shap_values # For regression, it's already 2D


# --- Bar summary plot (mean |SHAP|) ---
plt.figure(figsize=(8, 5))
shap.summary_plot(shap_vals_for_plotting, X_test_sub,
                  feature_names=feature_names,
                  plot_type='bar', show=False)
plt.title('SHAP Feature Importance – Non-Leaky DT Model')
plt.tight_layout()
...
shap.summary_plot(shap_vals_for_plotting, X_test_sub,
                  feature_names=feature_names,
                  show=False)
plt.title('SHAP Value Distribution – Non-Leaky DT Model')
plt.tight_layout()
plt.savefig('shap_beeswarm.png', dpi=150, bbox_inches='tight')
plt.show()
print("Saved: shap_beeswarm.png")

# Print top feature by mean |SHAP|
# Now mean_shap will be a 1D array of shape (n_features,)
mean_shap = np.abs(shap_vals_for_plotting).mean(axis=0)
for fn, ms in sorted(zip(feature_names, mean_shap), key=lambda x: -x[1]):
    print(f"  {fn}: mean |SHAP| = {ms:.4f}")

# ============================================================
# CELL: ROC Curve – Non-Leaky Decision Tree
# Add this after your existing ROC/AUC cell
# ============================================================
from sklearn.metrics import roc_curve, auc, RocCurveDisplay
import matplotlib.pyplot as plt

# Probability predictions on non-leaky test set
y_prob_sub = best_clf_sub.predict_proba(X_test_sub)[:, 1]

fpr, tpr, _ = roc_curve(y_test_sub, y_prob_sub)
roc_auc_sub = auc(fpr, tpr)

fig, ax = plt.subplots(figsize=(7, 6))
ax.plot(fpr, tpr, color='darkorange', lw=2,
        label=f'Non-Leaky DT  (AUC = {roc_auc_sub:.3f})')
ax.plot([0, 1], [0, 1], 'k--', lw=1, label='Random (AUC = 0.500)')
ax.set_xlim([0.0, 1.0])
ax.set_ylim([0.0, 1.05])
ax.set_xlabel('False Positive Rate', fontsize=12)
ax.set_ylabel('True Positive Rate', fontsize=12)
ax.set_title('ROC Curve – Non-Leaky Decision Tree', fontsize=13)
ax.legend(loc='lower right', fontsize=11)
plt.tight_layout()
plt.savefig('roc_curve_nonleaky.png', dpi=150, bbox_inches='tight')
plt.show()
print(f"Non-leaky AUC = {roc_auc_sub:.3f}")
print("Saved: roc_curve_nonleaky.png")

# ============================================================
# CELL: Learning Curve – explains CV F1 (0.529) vs test F1 (0.802)
# Add this after your cross-validation results cell
# ============================================================
...
# Use the same best parameters found by GridSearchCV
lc_clf = DecisionTreeClassifier(**best_clf_sub.get_params())
...
ax.plot(train_sizes, train_mean, 'o-', color='royalblue', label='Training F1')
ax.fill_between(train_sizes, train_mean - train_std,
                train_mean + train_std, alpha=0.15, color='royalblue')
ax.plot(train_sizes, val_mean, 'o-', color='seagreen', label='CV Validation F1')
ax.fill_between(train_sizes, val_mean - val_std,
                val_mean + val_std, alpha=0.15, color='seagreen')
ax.axhline(y=0.802, color='gray', linestyle='--', lw=1,
           label='Test F1 = 0.802')
ax.set_xlabel('Training Set Size', fontsize=12)
ax.set_ylabel('F1 Score', fontsize=12)
ax.set_title('Learning Curve – Non-Leaky Decision Tree', fontsize=13)
